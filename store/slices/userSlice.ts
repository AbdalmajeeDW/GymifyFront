  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchUsers.pending, (state) => {
        state.status = state.users && statusRequest.LOADING;
      })
      .addCase(fetchUsers.fulfilled, (state, action) => {
        state.users = action.payload;
        state.status = state.users && statusRequest.SUCCEEDED;
      })
      .addCase(fetchUsers.rejected, (state, action) => {
        state.status = state.users && statusRequest.FAILED;
      });
    builder
...
export const addOneUser = (state: { user: UserState }) => state.user.users;
export const statusAllUsers = (state: { user: UserState }) => state.user.status;
export const titlePage = (state: { user: UserState }) => state.user.titlePage;
export const addUserStatusSelector = (state: { user: UserState }) =>
  state.user.statusAddUser;
